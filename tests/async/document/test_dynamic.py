...
from mongoengine.async_document import AsyncDynamicDocument, AsyncDocument
...
class TestDynamicDocument(MongoDBTestCase):
    ...
    def test_dynamic_document_parse_values_in_constructor_like_document_do(self):
        class ProductDynamicDocument(AsyncDynamicDocument):
            title = StringField()
            price = FloatField()
    ...
    async def test_fields_containing_underscore(self):
        ...
        class WeirdPerson(AsyncDynamicDocument):
            name = StringField()
            _name = StringField()
