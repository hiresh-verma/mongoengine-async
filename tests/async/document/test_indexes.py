from datetime import datetime

import pytest
import pytest_asyncio
from pymongo.collation import Collation
from pymongo.errors import OperationFailure

from mongoengine import *
from mongoengine.async_document import AsyncDocument, AsyncDynamicDocument
from mongoengine.async_pymongo_support import async_list_collection_names
from mongoengine.io.aio.connection import (
    async_connect,
    async_disconnect_all,
    async_get_db,
)
from mongoengine.mongodb_support import (
    MONGODB_42,
    MONGODB_80,
    get_mongodb_version,
)
from mongoengine.pymongo_support import PYMONGO_VERSION


@pytest_asyncio.fixture
async def async_db():
    """Set up async database connection."""
    await async_disconnect_all()
    await async_connect(db="mongoenginetest_async_indexes")
    db = await async_get_db()

    yield db

    # Cleanup
    await async_disconnect_all()


@pytest.mark.asyncio
async def test_indexes_document(async_db):
    """Ensure that indexes are used when meta[indexes] is specified for
    Documents
    """
    await _index_test(AsyncDocument, async_db)


@pytest.mark.asyncio
async def test_indexes_dynamic_document(async_db):
    """Ensure that indexes are used when meta[indexes] is specified for
    Dynamic Documents
    """
    await _index_test(AsyncDynamicDocument, async_db)


async def _index_test(InheritFrom, async_db):
    class BlogPost(InheritFrom):
        date = DateTimeField(db_field="addDate", default=datetime.now)
        category = StringField()
        tags = ListField(StringField())
        meta = {"indexes": ["-date", "tags", ("category", "-date")]}

    expected_specs = [
        {"fields": [("addDate", -1)]},
        {"fields": [("tags", 1)]},
        {"fields": [("category", 1), ("addDate", -1)]},
    ]
    assert expected_specs == BlogPost._meta["index_specs"]

    await BlogPost.ensure_indexes()
    collection = await BlogPost._get_async_collection()
    info = await collection.index_information()
    # _id, '-date', 'tags', ('cat', 'date')
    assert len(info) == 4
    info_keys = [value["key"] for key, value in info.items()]
    for expected in expected_specs:
        assert expected["fields"] in info_keys

    assert await BlogPost.compare_indexes() == {"missing": [], "extra": []}


async def _index_test_inheritance(InheritFrom, async_db):
    class BlogPost(InheritFrom):
        date = DateTimeField(db_field="addDate", default=datetime.now)
        category = StringField()
        tags = ListField(StringField())
        meta = {
            "indexes": ["-date", "tags", ("category", "-date")],
            "allow_inheritance": True,
        }

    expected_specs = [
        {"fields": [("_cls", 1), ("addDate", -1)]},
        {"fields": [("_cls", 1), ("tags", 1)]},
        {"fields": [("_cls", 1), ("category", 1), ("addDate", -1)]},
    ]
    assert expected_specs == BlogPost._meta["index_specs"]

    await BlogPost.ensure_indexes()
    collection = await BlogPost._get_async_collection()
    info = await collection.index_information()
    # _id, '-date', 'tags', ('cat', 'date')
    # NB: there is no index on _cls by itself, since
    # the indices on -date and tags will both contain
    # _cls as first element in the key
    assert len(info) == 4
    info_keys = [value["key"] for key, value in info.items()]
    for expected in expected_specs:
        assert expected["fields"] in info_keys

    class ExtendedBlogPost(BlogPost):
        title = StringField()
        meta = {"indexes": ["title"]}

    expected_specs.append({"fields": [("_cls", 1), ("title", 1)]})
    assert expected_specs == ExtendedBlogPost._meta["index_specs"]

    await BlogPost.drop_collection()

    await ExtendedBlogPost.ensure_indexes()
    collection = await ExtendedBlogPost._get_async_collection()
    info = await collection.index_information()
    info_keys = [value["key"] for key, value in info.items()]
    for expected in expected_specs:
        assert expected["fields"] in info_keys


@pytest.mark.asyncio
async def test_indexes_document_inheritance(async_db):
    """Ensure that indexes are used when meta[indexes] is specified for
    Documents
    """
    await _index_test_inheritance(AsyncDocument, async_db)


@pytest.mark.asyncio
async def test_indexes_dynamic_document_inheritance(async_db):
    """Ensure that indexes are used when meta[indexes] is specified for
    Dynamic Documents
    """
    await _index_test_inheritance(AsyncDynamicDocument, async_db)


@pytest.mark.asyncio
async def test_inherited_index(async_db):
    """Ensure index specs are inherited correctly"""

    class A(AsyncDocument):
        title = StringField()
        meta = {"indexes": [{"fields": ("title",)}], "allow_inheritance": True}

    class B(A):
        description = StringField()

    assert A._meta["index_specs"] == B._meta["index_specs"]
    assert [{"fields": [("_cls", 1), ("title", 1)]}] == A._meta["index_specs"]


@pytest.mark.asyncio
async def test_index_no_cls(async_db):
    """Ensure index specs are inherited correctly"""

    class A(AsyncDocument):
        title = StringField()
        meta = {
            "indexes": [{"fields": ("title",), "cls": False}],
            "allow_inheritance": True,
            "index_cls": False,
        }

    assert [("title", 1)] == A._meta["index_specs"][0]["fields"]
    collection = await A._get_async_collection()
    await collection.drop_indexes()
    await A.ensure_indexes()
    info = await collection.index_information()
    assert len(info.keys()) == 2

    class B(A):
        c = StringField()
        d = StringField()
        meta = {
            "indexes": [{"fields": ["c"]}, {"fields": ["d"], "cls": True}],
            "allow_inheritance": True,
        }

    assert [("c", 1)] == B._meta["index_specs"][1]["fields"]
    assert [("_cls", 1), ("d", 1)] == B._meta["index_specs"][2]["fields"]


@pytest.mark.asyncio
async def test_build_index_spec_is_not_destructive(async_db):
    class MyDoc(AsyncDocument):
        keywords = StringField()

        meta = {"indexes": ["keywords"], "allow_inheritance": False}

    assert MyDoc._meta["index_specs"] == [{"fields": [("keywords", 1)]}]

    # Force index creation
    await MyDoc.ensure_indexes()

    assert MyDoc._meta["index_specs"] == [{"fields": [("keywords", 1)]}]


@pytest.mark.asyncio
async def test_embedded_document_index_meta(async_db):
    """Ensure that embedded document indexes are created explicitly"""

    class Rank(EmbeddedDocument):
        title = StringField(required=True)

    class Person(AsyncDocument):
        name = StringField(required=True)
        rank = EmbeddedDocumentField(Rank, required=False)

        meta = {"indexes": ["rank.title"], "allow_inheritance": False}

    assert [{"fields": [("rank.title", 1)]}] == Person._meta["index_specs"]

    await Person.drop_collection()

    # Indexes are lazy so use async for to perform query
    async for _ in Person.objects:
        pass
    collection = await Person._get_async_collection()
    info = await collection.index_information()
    info_keys = [value["key"] for key, value in info.items()]
    assert [("rank.title", 1)] in info_keys


@pytest.mark.asyncio
async def test_explicit_geo2d_index(async_db):
    """Ensure that geo2d indexes work when created via meta[indexes]"""

    class Place(AsyncDocument):
        location = DictField()
        meta = {"allow_inheritance": True, "indexes": ["*location.point"]}

    assert [{"fields": [("location.point", "2d")]}] == Place._meta["index_specs"]

    await Place.ensure_indexes()
    collection = await Place._get_async_collection()
    info = await collection.index_information()
    info_keys = [value["key"] for key, value in info.items()]
    assert [("location.point", "2d")] in info_keys


@pytest.mark.asyncio
async def test_explicit_geo2d_index_embedded(async_db):
    """Ensure that geo2d indexes work when created via meta[indexes]"""

    class EmbeddedLocation(EmbeddedDocument):
        location = DictField()

    class Place(AsyncDocument):
        current = DictField(field=EmbeddedDocumentField("EmbeddedLocation"))
        meta = {"allow_inheritance": True, "indexes": ["*current.location.point"]}

    assert [{"fields": [("current.location.point", "2d")]}] == Place._meta[
        "index_specs"
    ]

    await Place.ensure_indexes()
    collection = await Place._get_async_collection()
    info = await collection.index_information()
    info_keys = [value["key"] for key, value in info.items()]
    assert [("current.location.point", "2d")] in info_keys


@pytest.mark.asyncio
async def test_explicit_geosphere_index(async_db):
    """Ensure that geosphere indexes work when created via meta[indexes]"""

    class Place(AsyncDocument):
        location = DictField()
        meta = {"allow_inheritance": True, "indexes": ["(location.point"]}

    assert [{"fields": [("location.point", "2dsphere")]}] == Place._meta[
        "index_specs"
    ]

    await Place.ensure_indexes()
    collection = await Place._get_async_collection()
    info = await collection.index_information()
    info_keys = [value["key"] for key, value in info.items()]
    assert [("location.point", "2dsphere")] in info_keys


@pytest.mark.asyncio
async def test_explicit_geohaystack_index(async_db):
    """Ensure that geohaystack indexes work when created via meta[indexes]"""
    # This test can be removed when pymongo 3.x is no longer supported
    if PYMONGO_VERSION >= (4,):
        pytest.skip("GEOHAYSTACK has been removed in pymongo 4.0")

    class Place(AsyncDocument):
        location = DictField()
        name = StringField()
        meta = {"indexes": [(")location.point", "name")]}

    assert [
        {"fields": [("location.point", "geoHaystack"), ("name", 1)]}
    ] == Place._meta["index_specs"]

    # GeoHaystack index creation is not supported for now from meta, as it
    # requires a bucketSize parameter.
    if False:
        await Place.ensure_indexes()
        collection = await Place._get_async_collection()
        info = await collection.index_information()
        info_keys = [value["key"] for key, value in info.items()]
        assert [("location.point", "geoHaystack")] in info_keys


@pytest.mark.asyncio
async def test_create_geohaystack_index(async_db):
    """Ensure that geohaystack indexes can be created"""

    class Place(AsyncDocument):
        location = DictField()
        name = StringField()

    if PYMONGO_VERSION >= (4,):
        expected_error = NotImplementedError
    elif get_mongodb_version() >= (4, 9):
        expected_error = OperationFailure
    else:
        expected_error = None

    # This test can be removed when pymongo 3.x is no longer supported
    if expected_error:
        with pytest.raises(expected_error):
            await Place.create_index(
                {"fields": (")location.point", "name")},
                bucketSize=10,
            )
    else:
        await Place.create_index({"fields": (")location.point", "name")}, bucketSize=10)
        collection = await Place._get_async_collection()
        info = await collection.index_information()
        info_keys = [value["key"] for key, value in info.items()]
        assert [("location.point", "geoHaystack"), ("name", 1)] in info_keys


@pytest.mark.asyncio
async def test_dictionary_indexes(async_db):
    """Ensure that indexes are used when meta[indexes] contains
    dictionaries instead of lists.
    """

    class BlogPost(AsyncDocument):
        date = DateTimeField(db_field="addDate", default=datetime.now)
        category = StringField()
        tags = ListField(StringField())
        meta = {"indexes": [{"fields": ["-date"], "unique": True, "sparse": True}]}

    assert [
        {"fields": [("addDate", -1)], "unique": True, "sparse": True}
    ] == BlogPost._meta["index_specs"]

    await BlogPost.drop_collection()

    collection = await BlogPost._get_async_collection()
    info = await collection.index_information()
    # _id, '-date'
    assert len(info) == 2

    # Indexes are lazy so use async for to perform query
    async for _ in BlogPost.objects:
        pass
    info = await collection.index_information()
    info_data = [
        (value["key"], value.get("unique", False), value.get("sparse", False))
        for key, value in info.items()
    ]
    assert ([("addDate", -1)], True, True) in info_data

    await BlogPost.drop_collection()


@pytest.mark.asyncio
async def test_abstract_index_inheritance(async_db):
    class UserBase(AsyncDocument):
        user_guid = StringField(required=True)
        meta = {
            "abstract": True,
            "indexes": ["user_guid"],
            "allow_inheritance": True,
        }

    class Person(UserBase):
        name = StringField()

        meta = {"indexes": ["name"]}

    await Person.drop_collection()

    await Person(name="test", user_guid="123").save()

    assert 1 == await Person.objects.count()
    collection = await Person._get_async_collection()
    info = await collection.index_information()
    assert sorted(info.keys()) == ["_cls_1_name_1", "_cls_1_user_guid_1", "_id_"]


@pytest.mark.asyncio
async def test_disable_index_creation(async_db):
    """Tests setting auto_create_index to False on the connection will
    disable any index generation.
    """

    class User(AsyncDocument):
        meta = {
            "allow_inheritance": True,
            "indexes": ["user_guid"],
            "auto_create_index": False,
        }
        user_guid = StringField(required=True)

    class MongoUser(User):
        pass

    await User.drop_collection()

    await User(user_guid="123").save()
    await MongoUser(user_guid="123").save()

    assert 2 == await User.objects.count()
    collection = await User._get_async_collection()
    info = await collection.index_information()
    assert list(info.keys()) == ["_id_"]

    await User.ensure_indexes()
    info = await collection.index_information()
    assert sorted(info.keys()) == ["_cls_1_user_guid_1", "_id_"]


@pytest.mark.asyncio
async def test_embedded_document_index(async_db):
    """Tests settings an index on an embedded document"""

    class Date(EmbeddedDocument):
        year = IntField(db_field="yr")

    class BlogPost(AsyncDocument):
        title = StringField()
        date = EmbeddedDocumentField(Date)

        meta = {"indexes": ["-date.year"]}

    await BlogPost.drop_collection()

    collection = await BlogPost._get_async_collection()
    info = await collection.index_information()
    assert sorted(info.keys()) == ["_id_", "date.yr_-1"]


@pytest.mark.asyncio
async def test_list_embedded_document_index(async_db):
    """Ensure list embedded documents can be indexed"""

    class Tag(EmbeddedDocument):
        name = StringField(db_field="tag")

    class BlogPost(AsyncDocument):
        title = StringField()
        tags = ListField(EmbeddedDocumentField(Tag))

        meta = {"indexes": ["tags.name"]}

    await BlogPost.drop_collection()

    collection = await BlogPost._get_async_collection()
    info = await collection.index_information()
    # we don't use _cls in with list fields by default
    assert sorted(info.keys()) == ["_id_", "tags.tag_1"]

    post1 = BlogPost(
        title="Embedded Indexes tests in place",
        tags=[Tag(name="about"), Tag(name="time")],
    )
    await post1.save()


@pytest.mark.asyncio
async def test_recursive_embedded_objects_dont_break_indexes(async_db):
    class RecursiveObject(EmbeddedDocument):
        obj = EmbeddedDocumentField("self")

    class RecursiveDocument(AsyncDocument):
        recursive_obj = EmbeddedDocumentField(RecursiveObject)
        meta = {"allow_inheritance": True}

    await RecursiveDocument.ensure_indexes()
    collection = await RecursiveDocument._get_async_collection()
    info = await collection.index_information()
    assert sorted(info.keys()) == ["_cls_1", "_id_"]


@pytest.mark.asyncio
async def test_covered_index(async_db):
    """Ensure that covered indexes can be used"""

    class Test(AsyncDocument):
        a = IntField()
        b = IntField()

        meta = {"indexes": ["a"], "allow_inheritance": False}

    await Test.drop_collection()

    obj = Test(a=1)
    await obj.save()

    # Need to be explicit about covered indexes as mongoDB doesn't know if
    # the documents returned might have more keys in that here.
    mongo_db = get_mongodb_version()
    if mongo_db >= MONGODB_80:
        query_plan = await Test.objects(id=obj.id).exclude("a").explain()
        assert (
            query_plan["queryPlanner"]["winningPlan"]["stage"] == "EXPRESS_IXSCAN"
        )

        query_plan = await Test.objects(id=obj.id).only("id").explain()
        assert (
            query_plan["queryPlanner"]["winningPlan"]["stage"] == "EXPRESS_IXSCAN"
        )

        query_plan = await Test.objects(a=1).only("a").exclude("id").explain()
        assert (
            query_plan["queryPlanner"]["winningPlan"]["inputStage"]["stage"]
            == "IXSCAN"
        )
        assert (
            query_plan["queryPlanner"]["winningPlan"]["stage"]
            == "PROJECTION_COVERED"
        )

        query_plan = await Test.objects(a=1).explain()
        assert (
            query_plan["queryPlanner"]["winningPlan"]["inputStage"]["stage"]
            == "IXSCAN"
        )

        assert (
            query_plan.get("queryPlanner").get("winningPlan").get("stage")
            == "FETCH"
        )
    elif mongo_db < MONGODB_80:
        query_plan = await Test.objects(id=obj.id).exclude("a").explain()
        assert (
            query_plan["queryPlanner"]["winningPlan"]["inputStage"]["stage"]
            == "IDHACK"
        )

        query_plan = await Test.objects(id=obj.id).only("id").explain()
        assert (
            query_plan["queryPlanner"]["winningPlan"]["inputStage"]["stage"]
            == "IDHACK"
        )

        query_plan = await Test.objects(a=1).only("a").exclude("id").explain()
        assert (
            query_plan["queryPlanner"]["winningPlan"]["inputStage"]["stage"]
            == "IXSCAN"
        )

        PROJECTION_STR = (
            "PROJECTION" if mongo_db < MONGODB_42 else "PROJECTION_COVERED"
        )
        assert query_plan["queryPlanner"]["winningPlan"]["stage"] == PROJECTION_STR

        query_plan = await Test.objects(a=1).explain()
        assert (
            query_plan["queryPlanner"]["winningPlan"]["inputStage"]["stage"]
            == "IXSCAN"
        )

        assert (
            query_plan.get("queryPlanner").get("winningPlan").get("stage")
            == "FETCH"
        )


@pytest.mark.asyncio
async def test_index_on_id(async_db):
    class BlogPost(AsyncDocument):
        meta = {"indexes": [["categories", "id"]]}

        title = StringField(required=True)
        description = StringField(required=True)
        categories = ListField()

    await BlogPost.drop_collection()

    collection = await BlogPost._get_async_collection()
    indexes = await collection.index_information()
    assert indexes["categories_1__id_1"]["key"] == [("categories", 1), ("_id", 1)]


@pytest.mark.asyncio
async def test_hint(async_db):
    TAGS_INDEX_NAME = "tags_1"

    class BlogPost(AsyncDocument):
        tags = ListField(StringField())
        meta = {"indexes": [{"fields": ["tags"], "name": TAGS_INDEX_NAME}]}

    await BlogPost.drop_collection()

    for i in range(10):
        tags = [("tag %i" % n) for n in range(i % 2)]
        await BlogPost(tags=tags).save()

    # Hinting by shape should work.
    assert await BlogPost.objects.hint([("tags", 1)]).count() == 10

    # Hinting by index name should work.
    assert await BlogPost.objects.hint(TAGS_INDEX_NAME).count() == 10

    # Clearing the hint should work fine.
    assert await BlogPost.objects.hint().count() == 10
    assert await BlogPost.objects.hint([("ZZ", 1)]).hint().count() == 10

    # Hinting on a non-existent index shape should fail.
    with pytest.raises(OperationFailure):
        await BlogPost.objects.hint([("ZZ", 1)]).count()

    # Hinting on a non-existent index name should fail.
    with pytest.raises(OperationFailure):
        await BlogPost.objects.hint("Bad Name").count()

    # Invalid shape argument (missing list brackets) should fail.
    if PYMONGO_VERSION <= (4, 3):
        with pytest.raises(ValueError):
            await BlogPost.objects.hint(("tags", 1)).count()
    else:
        with pytest.raises(TypeError):
            await BlogPost.objects.hint(("tags", 1)).count()


@pytest.mark.asyncio
async def test_collation(async_db):
    base = {"locale": "en", "strength": 2}

    class BlogPost(AsyncDocument):
        name = StringField()
        meta = {
            "indexes": [
                {"fields": ["name"], "name": "name_index", "collation": base}
            ]
        }

    await BlogPost.drop_collection()

    names = ["tag1", "Tag2", "tag3", "Tag4", "tag5"]
    for name in names:
        await BlogPost(name=name).save()

    query_result = BlogPost.objects.collation(base).order_by("name")
    result_names = []
    async for x in query_result:
        result_names.append(x.name)
    assert result_names == sorted(names, key=lambda x: x.lower())
    assert 5 == await query_result.count()

    query_result = BlogPost.objects.collation(Collation(**base)).order_by("name")
    result_names = []
    async for x in query_result:
        result_names.append(x.name)
    assert result_names == sorted(names, key=lambda x: x.lower())
    assert 5 == await query_result.count()

    incorrect_collation = {"arndom": "wrdo"}
    with pytest.raises(OperationFailure) as exc_info:
        await BlogPost.objects.collation(incorrect_collation).count()
    assert "Missing expected field" in str(
        exc_info.value
    ) or "unknown field" in str(exc_info.value)

    query_result = BlogPost.objects.collation({}).order_by("name")
    result_names = []
    async for x in query_result:
        result_names.append(x.name)
    assert result_names == sorted(names)


@pytest.mark.asyncio
async def test_unique(async_db):
    """Ensure that uniqueness constraints are applied to fields."""

    class BlogPost(AsyncDocument):
        title = StringField()
        slug = StringField(unique=True)

    await BlogPost.drop_collection()

    post1 = BlogPost(title="test1", slug="test")
    await post1.save()

    # Two posts with the same slug is not allowed
    post2 = BlogPost(title="test2", slug="test")
    with pytest.raises(NotUniqueError):
        await post2.save()
    with pytest.raises(NotUniqueError):
        await BlogPost.objects.insert(post2)

    # Ensure backwards compatibility for errors
    with pytest.raises(OperationError):
        await post2.save()


@pytest.mark.asyncio
async def test_primary_key_unique_not_working(async_db):
    """Relates to #1445"""

    class Blog(AsyncDocument):
        id = StringField(primary_key=True, unique=True)

    await Blog.drop_collection()

    with pytest.raises(OperationFailure) as exc_info:
        await Blog(id="garbage").save()

    # One of the errors below should happen. Which one depends on the
    # PyMongo version and dict order.
    err_msg = str(exc_info.value)
    assert any(
        [
            "The field 'unique' is not valid for an _id index specification"
            in err_msg,
            "The field 'background' is not valid for an _id index specification"
            in err_msg,
            "The field 'sparse' is not valid for an _id index specification"
            in err_msg,
        ]
    )


@pytest.mark.asyncio
async def test_unique_with(async_db):
    """Ensure that unique_with constraints are applied to fields."""

    class Date(EmbeddedDocument):
        year = IntField(db_field="yr")

    class BlogPost(AsyncDocument):
        title = StringField()
        date = EmbeddedDocumentField(Date)
        slug = StringField(unique_with="date.year")

    await BlogPost.drop_collection()

    post1 = BlogPost(title="test1", date=Date(year=2009), slug="test")
    await post1.save()

    # day is different so won't raise exception
    post2 = BlogPost(title="test2", date=Date(year=2010), slug="test")
    await post2.save()

    # Now there will be two docs with the same slug and the same day: fail
    post3 = BlogPost(title="test3", date=Date(year=2010), slug="test")
    with pytest.raises(OperationError):
        await post3.save()


@pytest.mark.asyncio
async def test_unique_embedded_document(async_db):
    """Ensure that uniqueness constraints are applied to fields on embedded documents."""

    class SubDocument(EmbeddedDocument):
        year = IntField(db_field="yr")
        slug = StringField(unique=True)

    class BlogPost(AsyncDocument):
        title = StringField()
        sub = EmbeddedDocumentField(SubDocument)

    await BlogPost.drop_collection()

    post1 = BlogPost(title="test1", sub=SubDocument(year=2009, slug="test"))
    await post1.save()

    # sub.slug is different so won't raise exception
    post2 = BlogPost(title="test2", sub=SubDocument(year=2010, slug="another-slug"))
    await post2.save()

    # Now there will be two docs with the same sub.slug
    post3 = BlogPost(title="test3", sub=SubDocument(year=2010, slug="test"))
    with pytest.raises(NotUniqueError):
        await post3.save()


@pytest.mark.asyncio
async def test_unique_embedded_document_in_list(async_db):
    """
    Ensure that the uniqueness constraints are applied to fields in
    embedded documents, even when the embedded documents in in a
    list field.
    """

    class SubDocument(EmbeddedDocument):
        year = IntField(db_field="yr")
        slug = StringField(unique=True)

    class BlogPost(AsyncDocument):
        title = StringField()
        subs = ListField(EmbeddedDocumentField(SubDocument))

    await BlogPost.drop_collection()

    post1 = BlogPost(
        title="test1",
        subs=[
            SubDocument(year=2009, slug="conflict"),
            SubDocument(year=2009, slug="conflict"),
        ],
    )
    await post1.save()

    post2 = BlogPost(title="test2", subs=[SubDocument(year=2014, slug="conflict")])

    with pytest.raises(NotUniqueError):
        await post2.save()


@pytest.mark.asyncio
async def test_unique_embedded_document_in_sorted_list(async_db):
    """
    Ensure that the uniqueness constraints are applied to fields in
    embedded documents, even when the embedded documents in a sorted list
    field.
    """

    class SubDocument(EmbeddedDocument):
        year = IntField()
        slug = StringField(unique=True)

    class BlogPost(AsyncDocument):
        title = StringField()
        subs = SortedListField(EmbeddedDocumentField(SubDocument), ordering="year")

    await BlogPost.drop_collection()

    post1 = BlogPost(
        title="test1",
        subs=[
            SubDocument(year=2009, slug="conflict"),
            SubDocument(year=2009, slug="conflict"),
        ],
    )
    await post1.save()

    # confirm that the unique index is created
    collection = await BlogPost._get_async_collection()
    indexes = await collection.index_information()
    assert "subs.slug_1" in indexes
    assert indexes["subs.slug_1"]["unique"]

    post2 = BlogPost(title="test2", subs=[SubDocument(year=2014, slug="conflict")])

    with pytest.raises(NotUniqueError):
        await post2.save()


@pytest.mark.asyncio
async def test_unique_embedded_document_in_embedded_document_list(async_db):
    """
    Ensure that the uniqueness constraints are applied to fields in
    embedded documents, even when the embedded documents in an embedded
    list field.
    """

    class SubDocument(EmbeddedDocument):
        year = IntField()
        slug = StringField(unique=True)

    class BlogPost(AsyncDocument):
        title = StringField()
        subs = EmbeddedDocumentListField(SubDocument)

    await BlogPost.drop_collection()

    post1 = BlogPost(
        title="test1",
        subs=[
            SubDocument(year=2009, slug="conflict"),
            SubDocument(year=2009, slug="conflict"),
        ],
    )
    await post1.save()

    # confirm that the unique index is created
    collection = await BlogPost._get_async_collection()
    indexes = await collection.index_information()
    assert "subs.slug_1" in indexes
    assert indexes["subs.slug_1"]["unique"]

    post2 = BlogPost(title="test2", subs=[SubDocument(year=2014, slug="conflict")])

    with pytest.raises(NotUniqueError):
        await post2.save()


@pytest.mark.asyncio
async def test_unique_with_embedded_document_and_embedded_unique(async_db):
    """Ensure that uniqueness constraints are applied to fields on
    embedded documents.  And work with unique_with as well.
    """

    class SubDocument(EmbeddedDocument):
        year = IntField(db_field="yr")
        slug = StringField(unique=True)

    class BlogPost(AsyncDocument):
        title = StringField(unique_with="sub.year")
        sub = EmbeddedDocumentField(SubDocument)

    await BlogPost.drop_collection()

    post1 = BlogPost(title="test1", sub=SubDocument(year=2009, slug="test"))
    await post1.save()

    # sub.slug is different so won't raise exception
    post2 = BlogPost(title="test2", sub=SubDocument(year=2010, slug="another-slug"))
    await post2.save()

    # Now there will be two docs with the same sub.slug
    post3 = BlogPost(title="test3", sub=SubDocument(year=2010, slug="test"))
    with pytest.raises(NotUniqueError):
        await post3.save()

    # Now there will be two docs with the same title and year
    post3 = BlogPost(title="test1", sub=SubDocument(year=2009, slug="test-1"))
    with pytest.raises(NotUniqueError):
        await post3.save()


@pytest.mark.asyncio
async def test_ttl_indexes(async_db):
    class Log(AsyncDocument):
        created = DateTimeField(default=datetime.now)
        meta = {"indexes": [{"fields": ["created"], "expireAfterSeconds": 3600}]}

    await Log.drop_collection()

    # Indexes are lazy so use async for to perform query
    async for _ in Log.objects:
        pass
    collection = await Log._get_async_collection()
    info = await collection.index_information()
    assert 3600 == info["created_1"]["expireAfterSeconds"]


@pytest.mark.asyncio
async def test_unique_and_indexes(async_db):
    """Ensure that 'unique' constraints aren't overridden by
    meta.indexes.
    """

    class Customer(AsyncDocument):
        cust_id = IntField(unique=True, required=True)
        meta = {"indexes": ["cust_id"], "allow_inheritance": False}

    await Customer.drop_collection()
    cust = Customer(cust_id=1)
    await cust.save()

    cust_dupe = Customer(cust_id=1)
    with pytest.raises(NotUniqueError):
        await cust_dupe.save()

    cust = Customer(cust_id=2)
    await cust.save()

    # duplicate key on update
    with pytest.raises(NotUniqueError):
        cust.cust_id = 1
        await cust.save()


@pytest.mark.asyncio
async def test_primary_save_duplicate_update_existing_object(async_db):
    """If you set a field as primary, then unexpected behaviour can occur.
    You won't create a duplicate but you will update an existing document.
    """

    class User(AsyncDocument):
        name = StringField(primary_key=True)
        password = StringField()

    await User.drop_collection()

    user = User(name="huangz", password="secret")
    await user.save()

    user = User(name="huangz", password="secret2")
    await user.save()

    assert await User.objects.count() == 1
    assert (await User.objects.first()).password == "secret2"


@pytest.mark.asyncio
async def test_unique_and_primary_create(async_db):
    """Create a new record with a duplicate primary key
    throws an exception
    """

    class User(AsyncDocument):
        name = StringField(primary_key=True)
        password = StringField()

    await User.drop_collection()

    await User.objects.create(name="huangz", password="secret")
    with pytest.raises(NotUniqueError):
        await User.objects.create(name="huangz", password="secret2")

    assert await User.objects.count() == 1
    assert (await User.objects.first()).password == "secret"


@pytest.mark.asyncio
async def test_index_with_pk(async_db):
    """Ensure you can use `pk` as part of a query"""

    class Comment(EmbeddedDocument):
        comment_id = IntField(required=True)

    try:

        class BlogPost(AsyncDocument):
            comments = EmbeddedDocumentField(Comment)
            meta = {
                "indexes": [
                    {"fields": ["pk", "comments.comment_id"], "unique": True}
                ]
            }

    except UnboundLocalError:
        pytest.fail("Unbound local error at index + pk definition")

    collection = await BlogPost._get_async_collection()
    info = await collection.index_information()
    info_keys = [value["key"] for key, value in info.items()]
    index_item = [("_id", 1), ("comments.comment_id", 1)]
    assert index_item in info_keys


@pytest.mark.asyncio
async def test_compound_key_embedded(async_db):
    class CompoundKey(EmbeddedDocument):
        name = StringField(required=True)
        term = StringField(required=True)

    class ReportEmbedded(AsyncDocument):
        key = EmbeddedDocumentField(CompoundKey, primary_key=True)
        text = StringField()

    my_key = CompoundKey(name="n", term="ok")
    report = await ReportEmbedded(text="OK", key=my_key).save()

    assert {"text": "OK", "_id": {"term": "ok", "name": "n"}} == report.to_mongo()
    assert report == await ReportEmbedded.objects.get(pk=my_key)


@pytest.mark.asyncio
async def test_compound_key_dictfield(async_db):
    class ReportDictField(AsyncDocument):
        key = DictField(primary_key=True)
        text = StringField()

    my_key = {"name": "n", "term": "ok"}
    report = await ReportDictField(text="OK", key=my_key).save()

    assert {"text": "OK", "_id": {"term": "ok", "name": "n"}} == report.to_mongo()

    # We can't directly call ReportDictField.objects.get(pk=my_key),
    # because dicts are unordered, and if the order in MongoDB is
    # different than the one in `my_key`, this test will fail.
    assert report == await ReportDictField.objects.get(pk__name=my_key["name"])
    assert report == await ReportDictField.objects.get(pk__term=my_key["term"])


@pytest.mark.asyncio
async def test_string_indexes(async_db):
    class MyDoc(AsyncDocument):
        provider_ids = DictField()
        meta = {"indexes": ["provider_ids.foo", "provider_ids.bar"]}

    collection = await MyDoc._get_async_collection()
    info = await collection.index_information()
    info_keys = [value["key"] for key, value in info.items()]
    assert [("provider_ids.foo", 1)] in info_keys
    assert [("provider_ids.bar", 1)] in info_keys


@pytest.mark.asyncio
async def test_sparse_compound_indexes(async_db):
    class MyDoc(AsyncDocument):
        provider_ids = DictField()
        meta = {
            "indexes": [
                {"fields": ("provider_ids.foo", "provider_ids.bar"), "sparse": True}
            ]
        }

    collection = await MyDoc._get_async_collection()
    info = await collection.index_information()
    assert [("provider_ids.foo", 1), ("provider_ids.bar", 1)] == info[
        "provider_ids.foo_1_provider_ids.bar_1"
    ]["key"]
    assert info["provider_ids.foo_1_provider_ids.bar_1"]["sparse"]

    assert await MyDoc.compare_indexes() == {"missing": [], "extra": []}


@pytest.mark.asyncio
async def test_text_indexes(async_db):
    class Book(AsyncDocument):
        title = DictField()
        meta = {"indexes": ["$title"]}

    collection = await Book._get_async_collection()
    indexes = await collection.index_information()
    assert "title_text" in indexes
    key = indexes["title_text"]["key"]
    assert ("_fts", "text") in key


@pytest.mark.asyncio
async def test_hashed_indexes(async_db):
    class Book(AsyncDocument):
        ref_id = StringField()
        meta = {"indexes": ["#ref_id"]}

    collection = await Book._get_async_collection()
    indexes = await collection.index_information()
    assert "ref_id_hashed" in indexes
    assert ("ref_id", "hashed") in indexes["ref_id_hashed"]["key"]

    assert await Book.compare_indexes() == {"missing": [], "extra": []}


@pytest.mark.asyncio
async def test_indexes_after_database_drop(async_db):
    """
    Test to ensure that indexes are not re-created on a collection
    after the database has been dropped unless auto_create_index_on_save
    is enabled.

    Issue #812 and #1446.
    """
    # Use a new connection and database since dropping the database could
    # cause concurrent tests to fail.
    tmp_alias = "test_indexes_after_database_drop_async"
    await async_disconnect_all()
    connection = await async_connect(db="tempdatabase_async", alias=tmp_alias)

    class BlogPost(AsyncDocument):
        slug = StringField(unique=True)
        meta = {"db_alias": tmp_alias}

    await BlogPost.drop_collection()
    await BlogPost(slug="test").save()
    with pytest.raises(NotUniqueError):
        await BlogPost(slug="test").save()

    # Drop the Database
    db = await async_get_db(tmp_alias)
    await connection.drop_database(db.name)
    await BlogPost(slug="test").save()
    # No error because the index was not recreated after dropping the database.
    await BlogPost(slug="test").save()

    # Repeat with auto_create_index_on_save: True.
    class BlogPost2(AsyncDocument):
        slug = StringField(unique=True)
        meta = {
            "db_alias": tmp_alias,
            "auto_create_index_on_save": True,
        }

    await BlogPost2.drop_collection()
    await BlogPost2(slug="test").save()
    with pytest.raises(NotUniqueError):
        await BlogPost2(slug="test").save()

    # Drop the Database
    await connection.drop_database(db.name)
    await BlogPost2(slug="test").save()
    # Error because ensure_indexes is run on every save().
    with pytest.raises(NotUniqueError):
        await BlogPost2(slug="test").save()

    await async_disconnect_all()


@pytest.mark.asyncio
async def test_index_dont_send_cls_option(async_db):
    """
    Ensure that 'cls' option is not sent through ensureIndex. We shouldn't
    send internal MongoEngine arguments that are not a part of the index
    spec.

    This is directly related to the fact that MongoDB doesn't validate the
    options that are passed to ensureIndex. For more details, see:
    https://jira.mongodb.org/browse/SERVER-769
    """

    class TestDoc(AsyncDocument):
        txt = StringField()

        meta = {
            "allow_inheritance": True,
            "indexes": [{"fields": ("txt",), "cls": False}],
        }

    class TestChildDoc(TestDoc):
        txt2 = StringField()

        meta = {"indexes": [{"fields": ("txt2",), "cls": False}]}

    await TestDoc.drop_collection()
    await TestDoc.ensure_indexes()
    await TestChildDoc.ensure_indexes()

    assert await TestDoc.compare_indexes() == {"missing": [], "extra": []}

    collection = await TestDoc._get_async_collection()
    index_info = await collection.index_information()
    for key in index_info:
        del index_info[key][
            "v"
        ]  # drop the index version - we don't care about that here
        if "ns" in index_info[key]:
            del index_info[key][
                "ns"
            ]  # drop the index namespace - we don't care about that here, MongoDB 3+

    assert index_info == {
        "txt_1": {"key": [("txt", 1)], "background": False},
        "_id_": {"key": [("_id", 1)]},
        "txt2_1": {"key": [("txt2", 1)], "background": False},
        "_cls_1": {"key": [("_cls", 1)], "background": False},
    }


@pytest.mark.asyncio
async def test_compound_index_underscore_cls_not_overwritten(async_db):
    """
    Test that the compound index doesn't get another _cls when it is specified
    """

    class TestDoc(AsyncDocument):
        shard_1 = StringField()
        txt_1 = StringField()

        meta = {
            "collection": "test",
            "allow_inheritance": True,
            "sparse": True,
            "shard_key": "shard_1",
            "indexes": [("shard_1", "_cls", "txt_1")],
        }

    await TestDoc.drop_collection()
    await TestDoc.ensure_indexes()

    assert await TestDoc.compare_indexes() == {"missing": [], "extra": []}

    collection = await TestDoc._get_async_collection()
    index_info = await collection.index_information()
    assert "shard_1_1__cls_1_txt_1_1" in index_info


@pytest.mark.asyncio
async def test_compare_indexes_works_with_compound_text_indexes(async_db):
    """The order of the fields in case of text indexes don't matter
    so it's important to ensure that the compare_indexes method works that way
    https://github.com/MongoEngine/mongoengine/issues/2612
    """

    class Sample1(AsyncDocument):
        a = StringField()
        b = StringField()

        meta = {"indexes": [{"fields": ["$a", "$b"]}]}

    class Sample2(AsyncDocument):
        a = StringField()
        b = StringField()

        meta = {"indexes": [{"fields": ["$b", "$a"]}]}

    await Sample1.drop_collection()
    await Sample2.drop_collection()
    assert await Sample1.compare_indexes() == {"missing": [], "extra": []}
    assert await Sample2.compare_indexes() == {"missing": [], "extra": []}
